...
<div class="admin-title-row">
    <div>
        <h1 style="font-size: 2rem;">Edit Pengalaman</h1>
        <p style="color: var(--text-secondary); font-size: 0.9rem;">Formulir untuk memperbarui riwayat karir, magang, freelance, atau organisasi Anda.</p>
    </div>
    ...
    <div class="admin-form-split" style="grid-template-columns: 2fr 1fr;">
        ...
        <div class="admin-card">
            ...
            <div class="form-group">
                ...
                <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" placeholder="Contoh: Senior Full-Stack Engineer" value="{{ old('title', $experience->title) }}" required>
                @error('title')
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 16px;">
                <div class="form-group">
                    ...
                    <input type="text" name="company" id="company" class="form-control @error('company') is-invalid @enderror" placeholder="Contoh: Google Corp Indonesia" value="{{ old('company', $experience->company) }}" required>
                    @error('company')
                        <span style="color: #ef4444; font-size: 0.8rem; margin-top: 4px;">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="type" id="type" class="form-control" required>
                        <option value="work" {{ old('type', $experience->type) == 'work' ? 'selected' : '' }}>Pekerjaan (Work)</option>
                        <option value="internship" {{ old('type', $experience->type) == 'internship' ? 'selected' : '' }}>Magang (Internship)</option>
                        <option value="freelance" {{ old('type', $experience->type) == 'freelance' ? 'selected' : '' }}>Freelance</option>
                        <option value="organization" {{ old('type', $experience->type) == 'organization' ? 'selected' : '' }}>Organisasi (Organization)</option>
                    </select>
                </div>
            </div>
            ...
            <div class="form-group" style="margin-top: 20px;">
                ...
                <textarea name="description" id="description" rows="6" class="form-control" placeholder="Jelaskan kontribusi, pencapaian, dan apa saja yang Anda kerjakan di posisi ini..." required>{{ old('description', $experience->description) }}</textarea>
            </div>
        </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 24px;">
            <div class="admin-card">
                ...
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 24px; font-size: 0.95rem;">
                    <input type="checkbox" name="is_current" id="is_current" value="1" style="width: 18px; height: 18px; accent-color: var(--accent); cursor: pointer;" {{ old('is_current', $experience->is_current) ? 'checked' : '' }}>
                    <label for="is_current" style="cursor: pointer; font-weight: 500;">Masih Bekerja Di Sini (Current)</label>
                </div>
                
                <button type="submit" class="btn btn-primary" style="width: 100%;"><i class="fa-solid fa-floppy-disk"></i> Perbarui Pengalaman</button>
            </div>
        </div>
    </div>
</form>
